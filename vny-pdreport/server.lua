...
AddEventHandler('veny: openAllReports', function()
    local idJ = source
    ...
    for i,k in pairs(jobs) do
        if job == k then
            ...
            function(result)
                TriggerClientEvent('veny: openAllReports', idJ, result)
            end)
...
AddEventHandler('veny: openReport', function()
    local source = source
    ...
    TriggerClientEvent('veny: openReport', source, name)
end)

RegisterNetEvent('veny: saveReport')
AddEventHandler('veny: saveReport', function(data)
    local idJ = source
    local xPlayer = ESX.GetPlayerFromId(idJ)
    ...
    MySQL.Async.execute('DELETE FROM policereports WHERE identifier = @identifier', {
        ...
        MySQL.Async.insert('INSERT INTO policereports (identifier, name, number, description) VALUES (@identifier, @name, @number, @description)',
        {
            ...
            TriggerClientEvent("veny: PoliceReportNotify", idJ, translate.TR_NOTIFY_REPORT)
            ...
            if insertedId ~= 0 then
                ...
                for i=1, #xPlayers, 1 do
                    ...
                    for j,k in pairs(jobs) do
                        if job == k then
                            TriggerClientEvent('veny: PoliceReportNotify', xPlayers[i] , translate.TR_NEW_REPORT)
                            break
                        end
                    end
                end
